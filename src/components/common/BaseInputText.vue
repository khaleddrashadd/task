<template>
  <div class="relative" :class="{ 'h-[36px]': sm }">
    <InputText
      ...
      :pt="{
        ...
        input: {
          class: `focus-visible:outline-0 ${readonly ? 'cursor-pointer' : ''}`
        }
      }"
      ...
      :readonly="readonly"
    />
    <label :for="id" class="text-xs absolute left-4" :class="{ 'top-1': sm }">{{ label }}</label>
  </div>
</template>
